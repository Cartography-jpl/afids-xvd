...
HelpBrowser::HelpBrowser()
{
    if (!_helpBrowserInit) {   // one-time initialization
        XmRepTypeId id;
        static String ControlStyleNames[] = { "netscape", "mosaic",
					      "default" };
        ...
        id = XmRepTypeRegister("HelpBrowserControlStyleType",
				ControlStyleNames, 
			       (unsigned char*) ControlStyleValues,
				XtNumber(ControlStyleNames) );
        XmRepTypeAddReverse(id);
        _helpBrowserInit = 1;
  } 

    _helpBrowserCommand = 0;
    _helpBaseLocation = 0;
    _helpLocation = 0;
...
void HelpBrowser::run(Widget w)
{
    ...
    if (_helpBrowserCommand == 0 || strlen(_helpBrowserCommand) == 0) {
        std::cerr << "No help browser available!\n";
        return;
    }
    if (_helpBaseLocation == 0 || strlen(_helpBaseLocation) == 0)
        return;
    if (_helpLocation == 0 || strlen(_helpLocation) == 0)
        return;
...
void HelpBrowser::run(char *cmd, char *base, char *loc, Boolean hasRemote, 
...
// VMS version
////////////////////////////////////////////////////////
void HelpBrowser::run(char *cmd, char *url, Boolean hasRemote,
		      HelpBrowserControlStyleType controlStyle)
  // hasRemote = True; controlStyle = DEFAULT_BROWSER_HELP
