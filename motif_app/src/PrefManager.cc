...
PrefManager::PrefManager(Display *dpy, char *pathname)
{
   if (_firsttime) {
   ...
   if (pathname != NULL) {
      ...
      XrmCombineFileDatabase(pathname, &db, True);
...
int PrefManager::loadPrefFile(char *pathname)
{
   XrmDatabase db;
...
void PrefManager::savePrefFile(char *pathname)
{
   XrmDatabase db;
   ...
   XrmPutFileDatabase(db, pathname);
...
void PrefManager::writeResourcesToDB(XrmDatabase *db, PrefRec *rec)
{
   ...
   if (compiled) {			// Create table for access
      ...
      for (Cardinal i=0; i<rec->_num_resources; i++)
