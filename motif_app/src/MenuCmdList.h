...
class MenuCmdList : public CmdList {
  ...
  public:
    ...
    MenuCmdList();    
    MenuCmdList(char *);           // Construct an empty list
    virtual ~MenuCmdList();  // Destroys list, but not objects in list
    
    virtual void add( Cmd *, CmdValue=0 ); // Add a single Cmd object to list
    ...
    virtual void createMenuPane ( Widget menu, Widget option=0 );
