...
StatusType VicarImageFile::readHistoryLabel(char*& histLabel, int *maxsize)
{
  ...
  // get the filename without path for the top line
   ...
   for (subset = 0; subset < number_of_tasks; subset++) {
      ...
      status = zlinfo(_unit,"HISTORY","TASK",format,&dummy,&value.nelements,
      ...
      while (TRUE) {
         ...
         status = zlget(_unit,"HISTORY",key, value.data,
                "HIST",task_names[subset], "INSTANCE",instances[subset],
...
StatusType VicarImageFile::readPropertyLabel(char*& propLabel, int *maxsize)
{
   ...
   struct multival value;	// Struct that describes the value for the item
				// holds LENGTH from zlinfo... needed since we
				// need the len of the string (via STRLEN
				// optional) instead of the len of the int or
				// real, since everything is treated as a
				// string by the zlget calls.
   ...
   int status;
   char printbuf[MAX_LABEL_ITEM_SIZE+1];	// Buffer for printing
   char file[1200];                 // filename without path
				...
				char *Lb_string, int *Lb_int, float *Lb_real)
{
   ...
   status=zlinfo(_unit, "PROPERTY", Label, lbf, &length, &nel,
   ...
   if (!strcmp(lbf, "INT")) {
      label_type = INT_LB;
      status=zlget(_unit, "PROPERTY", Label, (char*) Lb_int,
   ...
   if (!strcmp(lbf, "REAL")) {
      label_type = REAL_LB;
      status=zlget(_unit, "PROPERTY", Label, (char*) Lb_real,
                     "FORMAT", lbf, "PROPERTY", Property, "ERR_ACT", "", NULL);
      if (status!=1)
         return (UNDEFINED_LB);		// item or property doesn't exist
   }
   if (!strcmp(lbf, "STRING")) {
      ...
      status=zlget(_unit, "PROPERTY", Label, Lb_string,
                    "FORMAT", lbf, "PROPERTY", Property, "ERR_ACT", "", NULL);
      if (status!=1)
...
static void flushKeyValuePair(char *printbuf, char*& labels, int *maxsize)
{
   if (maxsize) {
	...
	while ((strlen(labels) + strlen(printbuf) + 7) > (size_t) *maxsize) {
            *maxsize = 2*(*maxsize);
         ...
         *maxsize = MAX_IMAGE_LABEL_SIZE+1;
         if ((strlen(printbuf)+1) > (size_t) *maxsize)
                              ...
                              char *format, char *printbuf, char*& label, int *maxsize)
{
   ...
   if ((strlen(printbuf)!=0) && (strlen(printbuf)+length >= WIDTH)) {
      strcat(label, printbuf);		// flush old buffer
      strcat(label, "\n");
      printbuf[0] = '\0';
   }
...
static int gtgetlab(char * inp, int instance, char * filename, char ** labelstr, int * nl, int * ns)
{
  ...
  if (! (buf = (char *) malloc (1000002))) {
    printf ("malloc failed in gtgetlab\n");
    exit (1);
  }
   
  /* open file */
   
  status = zvunit(&geounit,inp,instance,"u_name", filename,NULL);
  ...
  len = strlen(buf);
  if (((*labelstr)=(char *)malloc(len+1))==NULL) {
    printf ("malloc failed in gtgetlab 2\n");
    exit (1);
  }
  strcpy(*labelstr,buf);
...
char * VicarImageFile::readGeoTiffLabelFromFile() {
  ...
  gtgetlab("in_file",1,_filename,&labelstr,&labnl,&labns);  

  labelBuf = (char *) malloc (strlen (labelstr) + 200);
...
MP VicarImageFile::readMPInfoFromFile() 
{
  ...
  if (_isOpened != True) {
    ...
    vicStatus = zvunit(&unit, "in_file", 1, "u_name", _filename, NULL);
    if (vicStatus != 1) {
           ...
           char *propName, int instance, int *maxsize)
{
   ...
   // Set current key to task name
   status = zlinfo(_unit,"PROPERTY","PROPERTY",format,&dummy,
   ...
   while (TRUE) {
      ...
      status = zlget(_unit,"PROPERTY",key,value.data,
...
StatusType VicarImageFile::readSysLabel(char*& sysLabel, int *maxsize)
{
   ...
   status = zlinfo(_unit,"SYSTEM","LBLSIZE",format,&dummy,&value.nelements,
         "STRLEN", &value.maxlength, NULL);
   if (status <= 0) return imFAILURE;
   ...
   while (TRUE) {
      ...
      status = zlget(_unit,"SYSTEM",key,value.data, "FORMAT","STRING",
             "ULEN", value.maxlength, "NELEMENT", value.nelements, NULL);
      if (status <= 0) continue;
