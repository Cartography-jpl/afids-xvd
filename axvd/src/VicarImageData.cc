...
StatusType VicarImageData::open(char *stringOfFilenames)
{
   ...
   while (input_string && (strlen(input_string) > 0)) {
      ...
      p = filename;
      if (*input_string == '.' && strcspn(input_string,"/:]") >= (size_t) len) {
...
{ 
  if (_files[0] != NULL) {
    ...
    if (_mpObject == NULL) {
      _geoTiffLabel = _files[0]->readGeoTiffLabelFromFile();
    }
  }
...
//     double map[6],invmap[6],corner[4];
{
  int i,vtype;
   ...
   p = strstr(labelstr,"GTRASTERTYPEGEOKEY=2");
   if (p!=0) voff = 1.0; else voff = 0.5;     /* 0.5 is the default also */
   p = strstr(labelstr,"MODELTRANSFORMATIONTAG=(");
   if (p!=0)
      {
	p += strlen ("MODELTRANSFORMATIONTAG=(");
	sscanf (p, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", map+1, map, &ddummy, map+2, map+4, map+3, &ddummy, map+5);
      }
   else
      {
      p = strstr(labelstr,"MODELTIEPOINTTAG=(");
      if (p==0) { ireturn = 0; goto closem; }
      p += strlen ("MODELTIEPOINTTAG=(");
      sscanf (p, "%lf,%lf,%lf,%lf,%lf", tie, tie+1, &ddummy, tie+2, tie+3);
      p = strstr(labelstr,"MODELPIXELSCALETAG=(");
      if (p==0) { ireturn = 0; goto closem; }
      p += strlen ("MODELPIXELSCALETAG=(");
      sscanf (p, "%lf,%lf", scale, scale+1);
...
Widget VicarImageData::firstChildToUnmanage = 0;
Widget VicarImageData::secondChildToUnmanage = 0;
				     ...
				     double *lon, int type)
{
  ...
  if (_mpObject != NULL) {
    if( mpxy2ll(_mpObject, line +1 , samp+1, lat, lon, type) )
      ...
      return 1;
  } else if (_geoTiffLabel != NULL) {
    int nl, ns;
    char * p;
    double t[6],tinv[6],corner[4];

    XtUnmanageChild (firstChildToUnmanage);
    XtUnmanageChild (secondChildToUnmanage);

    if (! (p = strstr (_geoTiffLabel, "\nNL="))) {
      *lat = *lon = 0.0;
      return 0;
    }
    ...
    if (geofix(_geoTiffLabel,t,tinv,nl,ns,corner) != 1) {
      *lat = *lon = 0.0;
      return 0;
    }

    /* line/samp are passed zero based */
    line ++;
    samp ++;

    * lon = t[0] * line + t[1] * samp + t[2];
    * lat = t[3] * line + t[4] * samp + t[5];
    return 1;
  }
