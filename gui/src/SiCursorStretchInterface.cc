...
void SiCursorStretchInterface::activate()
{
   ...
   if (_cursorPixmap == None)
    ...
    reinterpret_cast<char*>(cur_str_bits), cur_str_width, cur_str_height);
   if (_cursorMask == None)
      _cursorMask = XCreateBitmapFromData(XtDisplay(_iw), XtWindow(_iw),
    reinterpret_cast<char*>(cur_str_msk_bits), cur_str_msk_width, cur_str_msk_height);
   ...
   if (_saveCursor != 0) {	// don't change cursor if we can't set it back
