...
$! VPACK Version 1.9, Friday, August 15, 2003, 11:17:22
$!
...
class ImageData {
 ...
 protected:
   ...
   char	_inputDataSourceName[400];  // usually a filename
   ...
   virtual void setErrorMsg(char *);	// For subclasses to report errors
...
class ImageTile {
 ...
 public:
   ...
   ImageTile()
	: _tileOwnsBuffer(FALSE), _numberOfBuffers(0), _newBufferNeeded(True)
	{ } 
...
class ImagePixelType {
 ...
 protected:
   ...
   static char *_DTtoStr[imNUM_DATA_TYPES];
 ...
 public:
   ...
   char *getString() const { return _DTtoStr[_pixelType]; }
   ...
   friend int operator!=(ImagePixelType p1, ImagePixelType p2)
      {  return (p1._pixelType != p2._pixelType); }
   ImagePixelType& ImagePixelType::operator=(const ImagePixelType& p)
...
class ImageDataWrapper : public ImageData { 
  ...
  protected:
    ...
    char _reloadDataSourceName[400]; // like _inputDataSourceName but un-munged
...
class ImageLabel {
   ...
   public:
      ImageLabel(ImageData *, char *, char *);
      virtual ~ImageLabel();
