...
$! VPACK Version 1.9, Friday, April 12, 2002, 17:21:14
$!
...
void ImageData::setErrorMsg(char *msg)
{
  if (_errorMsg != NULL) 
...
StatusType VicarImageData::open(char *stringOfFilenames)
{
   ...
   while (input_string && (strlen(input_string) > 0)) {
      ...
      p = filename;
      if (*input_string == '.' && strcspn(input_string,"/:]") >= len) {
				...
				char *Lb_string, int *Lb_int, float *Lb_real)
{
   ...
   if (!strcmp(lbf, "INT")) {
      label_type = INT_LB;
      status=zlget(_unit, "PROPERTY", Label, Lb_int,
   ...
   if (!strcmp(lbf, "REAL")) {
      label_type = REAL_LB;
      status=zlget(_unit, "PROPERTY", Label, Lb_real,
                        "FORMAT", lbf, "PROPERTY", Property, "ERR_ACT", "", NULL);
      if (status!=1)
         return (UNDEFINED_LB);		// item or property doesn't exist
   }
...
static void flushKeyValuePair(char *printbuf, char*& labels, int *maxsize)
{
   if (maxsize) {
      if (*maxsize != 0) {    // check to dynamically allocating memory
         while ((strlen(labels) + strlen(printbuf) + 7) > *maxsize) {
            *maxsize = 2*(*maxsize);
...
MP VicarImageFile::readMPInfoFromFile() 
{
  ...
  if (_isOpened != True) {
    ...
    vicStatus = zvunit(&unit, "in_file", 1, "u_name", _filename, 0);
    if (vicStatus != 1) {
    ...
    vicStatus = zvopen(unit, "OP", "READ", "U_FORMAT", "BYTE", 0);
    if (vicStatus != 1) {
...
char *ImagePixelType::_DTtoStr[imNUM_DATA_TYPES] =
