...
for dir in "$x_includes" "${prefix}/include" /usr/include /usr/local/include \
           ...
           /usr/dt/*/include /opt/*/include /sw/include /usr/include/Motif* \
...
for dir in "$x_libraries" "${prefix}/lib" /usr/lib /usr/local/lib \
           ...
           /usr/dt/*/lib /opt/*/lib /sw/lib /usr/lib/Motif* \
...
if test "$with_motif" != "no"; then
	...
	AC_DEFINE(HAVE_MOTIF, [], [If we have motif])
else
        with_motif="no"
fi
#
...
LIBS="$lt_save_LIBS"
AM_CONDITIONAL([HAVE_MOTIF], [test "$with_motif" = "yes"])
])
