...
TpMatchManager::TpMatchManager(TpWindow *window)
{
	...
	static unsigned char PointSymbolShapeValues[] = { 
	    ...
	    CrossWithHole45, 
	    RectangleWithCrossesWithDot };
        id = XmRepTypeRegister(XtpRPointSymbolShapeType, PointSymbolShapeNames,
...
void TpMatchManager::readPointsIbis(char *filename)
{
    ...
    status = zvunit(&unit, "tp_file_1",  1, "u_name", _ibisFilename, NULL);
    if (status != OK) {
...
void TpMatchManager::writePointsIbis(char *filename)
{
    ...
    status = zvunit(&unit, "tp_file_2", 1, "u_name", _ibisFilename, NULL);
    if (status != OK) {
        theErrorManager->process(Error, "zvunit", "Cant process file");
	return;
    }
...
{
    int minValue;
    _pointQualMgr->getMinValue(n, minValue);
    int maxValue;
    _pointQualMgr->getMaxValue(n, maxValue);
