...
char *TpPointModel::_pointColor = "red";
char *TpPointModel::_pointColorSel = "green";
Boolean TpPointModel::_showLabel = True;
#ifndef __VMS
  char *TpPointModel::_colorCodeFilename = "$V2DATA/gui/ps1.ibis-2";
#else
  char *TpPointModel::_colorCodeFilename = "v2$data:[gui]ps1.ibis-2";
#endif
...
void TpPointModel::colorCodePointsGen(int n, float a, float b)
{
    ...
    status = zvunit(&unit, "in_file",  1, "u_name", filename, NULL);
    if (status != 1) return;
...
#ifndef __VMS
    char *filename = "$V2DATA/gui/ps1.ibis-2";
#else
    char *filename = "v2$data:[gui]ps1.ibis-2";
#endif
    // open IBIS file for reading
    status = zvunit(&unit, "in_file",  1, "u_name", filename, NULL);
